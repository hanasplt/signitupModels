# extract_landmarks_sequence_v3.py
import os
...
# CONFIGURATION
# -----------------------------
DATA_DIR = './data_videos'        # Folder structure: data_videos/{gesture_name}/*.mp4
OUTPUT_FILE = 'j_z.p'
SEQUENCE_LENGTH = 50
MOTION_THRESHOLD = 0.001          # Motion sensitivity (lower = more strict)
INCLUDE_IDLE = True               # Add "None" idle class
DEBUG_VISUALIZE = True            # 👈 Toggle visualization ON/OFF
PLOT_SAMPLE = True                # 👈 Plot one example sequence at the end
...
print("🎥 Extracting hand landmarks from videos...\n")
...
for gesture in os.listdir(DATA_DIR):
    ...
    for video_file in os.listdir(gesture_path):
        ...
        print(f"▶️ Processing: {gesture}/{video_file}")
        ...
        while True:
            ...
            if DEBUG_VISUALIZE:
                ...
                cv2.putText(preview, f"{gesture} ({frame_index})", (20, 40),
        ...
        print(f"✅ {gesture}: {len(sequence)} frames | Skipped: {skipped_frames}")
