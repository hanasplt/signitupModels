...
SAVE_DIR = "./web_demo/FAMILY"
ONNX_NAME = "gesture_lstm_FAMILY.onnx"
ENCODER_NAME = "label_encoder_FAMILY.pickle"
...
with open('./processed_data/dynamic_gestures_data_FAMILY.p','rb') as f:
...
np.save(os.path.join(SAVE_DIR, "norm_mean_FAMILY.npy"), mean)
np.save(os.path.join(SAVE_DIR, "norm_std_FAMILY.npy"), std)

# Save as .json for the Web App (ONNX Runtime)
# We use .tolist() to ensure it is a clean JSON array
import json

with open(os.path.join(SAVE_DIR, "norm_mean_FAMILY.json"), "w") as f:
    json.dump(mean.tolist(), f)

with open(os.path.join(SAVE_DIR, "norm_std_FAMILY.json"), "w") as f:
...
plt.ioff()
plt.savefig(os.path.join(SAVE_DIR, "training_history_FAMILY.jpg"), format='jpg', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "confusion_matrix_FAMILY.jpg"), format='jpg', dpi=150, bbox_inches='tight')
plt.show()
...
with open(os.path.join(SAVE_DIR, "labels_FAMILY.json"), "w") as f:
